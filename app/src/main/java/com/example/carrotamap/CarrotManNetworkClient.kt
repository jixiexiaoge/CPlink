...
class CarrotManNetworkClient(
    ...
    // 数据去重和增量更新管理
    private var lastSentData = JSONObject()
    private var lastHeartbeatData = JSONObject()
    private var dataChangeThreshold = 0.001 // 数据变化阈值
    ...
    private fun addDiscoveredDevice(device: DeviceInfo) {
        ...
        if (!discoveredDevices.containsKey(deviceKey)) {
            ...
            if (currentTargetDevice == null) {
                ...
                Log.i(TAG, "🚀 自动连接到新发现的设备")
                connectToDevice(device)
            } else {
                // 如果当前设备不活跃，考虑切换到新设备
                if (!currentTargetDevice!!.isActive()) {
                    Log.i(TAG, "🔄 当前设备不活跃，切换到新设备: ${device.ip}")
                    connectToDevice(device)
                } else {
                    Log.d(TAG, "⚠️ 已有活跃连接设备 ${currentTargetDevice?.ip}，保持当前连接")
                }
            }
        } else {
                ...
                try {
                    ...
                    currentTargetDevice?.let { device ->
                        ...
                        if (!discoveredDevices.containsKey(deviceKey)) {
                            ...
                            // 智能选择最佳备用设备
                            val bestDevice = selectBestAvailableDevice()
                            if (bestDevice != null) {
                                Log.i(TAG, "自动切换到最佳备用设备: $bestDevice")
    ...
    private suspend fun sendHeartbeat() = withContext(Dispatchers.IO) {
        ...
        // 检查心跳包数据是否有变化
        if (hasDataChanged(heartbeatData, lastHeartbeatData, isHeartbeat = true)) {
            sendDataPacket(heartbeatData)
            lastHeartbeatData = JSONObject(heartbeatData.toString())
            Log.v(TAG, "心跳包已发送，索引: $carrotIndex")
        } else {
            Log.v(TAG, "心跳包数据无变化，跳过发送")
        }
    }
    ...
    fun sendCarrotManData(carrotFields: CarrotManFields) {
        ...
        networkScope.launch {
            try {
                ...
                // 检查数据是否有变化，只发送变化的数据
                if (hasDataChanged(jsonData, lastSentData, isHeartbeat = false)) {
                    sendDataPacket(jsonData)
                    lastSentData = JSONObject(jsonData.toString())
                    onDataSent?.invoke(++totalPacketsSent)
                    Log.v(TAG, "CarrotMan数据包发送成功 #$totalPacketsSent (数据有变化)")
                } else {
                    Log.v(TAG, "CarrotMan数据无变化，跳过发送")
                }
            } catch (e: Exception) {
            ...
            while (isRunning) {
                try {
                    ...
                    val currentFields = carrotManFieldsState.value
                        ...
                        Log.v(TAG, "⏸️ 自动发送跳过: enabled=${autoSendEnabled.value}, 时间间隔=${System.currentTimeMillis() - lastSendTime}ms")
                    }
                    delay(sendInterval)
                } catch (e: Exception) {
     ...
     * 智能选择最佳可用设备
     */
    private fun selectBestAvailableDevice(): DeviceInfo? {
        ...
        Log.d(TAG, "🎯 选择最佳设备: $bestDevice (从${activeDevices.size}个活跃设备中)")
        return bestDevice
    }
    ...
    /**
     * 检查数据是否有变化，实现数据去重和增量更新
     * @param newData 新数据
     * @param lastData 上次发送的数据
     * @param isHeartbeat 是否为心跳包
     * @return true表示数据有变化，需要发送
     */
    private fun hasDataChanged(newData: JSONObject, lastData: JSONObject, isHeartbeat: Boolean): Boolean {
        if (lastData.length() == 0) {
            Log.d(TAG, "首次发送数据，跳过去重检查")
            return true
        }
        
        // 心跳包只检查关键字段
        if (isHeartbeat) {
            val newIndex = newData.optLong("carrotIndex", 0)
            val lastIndex = lastData.optLong("carrotIndex", 0)
        ...
        val keyFields = listOf(
            ...
            "nRoadLimitSpeed", "nSdiType", "nSdiSpeedLimit", "nSdiDist",
            "nTBTDist", "nTBTTurnType", "szTBTMainText",
            "latitude", "longitude", "heading", "nPosSpeed",
            "nGoPosDist", "nGoPosTime", "szPosRoadName"
        )
        
        for (field in keyFields) {
            val newValue = newData.opt(field)
            val lastValue = lastData.opt(field)
            
            // 数值类型使用阈值比较
            if (newValue is Number && lastValue is Number) {
                val diff = kotlin.math.abs(newValue.toDouble() - lastValue.toDouble())
                if (diff > dataChangeThreshold) {
                    Log.d(TAG, "数据变化检测: $field 从 $lastValue 变为 $newValue (差值: $diff)")
                    return true
                }
            }
            // 字符串类型直接比较
            else if (newValue != lastValue) {
                Log.d(TAG, "数据变化检测: $field 从 '$lastValue' 变为 '$newValue'")
                return true
            }
        }
        
        // 检查carrotIndex变化（确保数据包顺序）
        val newIndex = newData.optLong("carrotIndex", 0)
        val lastIndex = lastData.optLong("carrotIndex", 0)
        if (newIndex != lastIndex) {
            Log.d(TAG, "数据变化检测: carrotIndex 从 $lastIndex 变为 $newIndex")
            return true
        }
     ...
     * @return 增量更新数据包
     */
    private fun createIncrementalUpdate(newData: JSONObject, lastData: JSONObject): JSONObject {
        val incrementalData = JSONObject()
        
        // 始终包含基础字段
        incrementalData.put("carrotIndex", newData.optLong("carrotIndex"))
        incrementalData.put("epochTime", newData.optLong("epochTime"))
        incrementalData.put("timestamp", newData.optDouble("timestamp"))
        incrementalData.put("carrotCmd", newData.optString("carrotCmd"))
        ...
        for (field in keyFields) {
            ...
            if (newValue != lastValue) {
                incrementalData.put(field, newValue)
                Log.d(TAG, "增量更新字段: $field = $newValue")
            }
        }
